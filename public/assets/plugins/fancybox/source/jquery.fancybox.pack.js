...
(function(B, x, f, q) {
    ...
    f.extend(b, {
        ...
        defaults: {
            ...
            title: null,
            tpl: {
                // <br><td id="memo_before"></td><td><button class="btn btn-warning btn-xs" id="memo_button_before" onclick="memo_change_form();" style="float:right; width:25px; height:25px; margin-right:5px;"><i class="fa fa-pencil"></i></button></td><td><input class="hidden" type="text" id="memo_after" placeholder="메모를 입력해주세요." style="width:92%;"></td><td><button class="hidden btn btn-success btn-xs" id="memo_button_after" onclick="memo_change_form();" style="float:right; height:25px; margin-right:5px;"><i class="fa fa-check"></i>확인</button></td> <br>
...
function revise_txt() {
    document.getElementById("memo").innerHTML = "<input id='memo' type='text' placeholder='메모를 입력해주세요.' style='width:920px; color:black;'>";
}

function memo_change_form(){
  d3.select("#memo_before").classed("hidden", true);
  d3.select("#memo_after").classed("hidden", false);

  d3.select("#memo_button_before").classed("hidden", true);
  d3.select("#memo_button_after").classed("hidden", false);
}